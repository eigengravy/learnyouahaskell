...
stepReverseSign num step = (abs num + step) * (if num > 0 then -1 else 1)
...
piCalc' :: (Ord a, Fractional a, Integral b) => a -> a -> a -> b -> (a, b)
piCalc' w x y z = if abs (pi - x) < y then (x, z) else piCalc' (stepReverseSign w 2) pi y (z + 1) where pi = x + 4 / w